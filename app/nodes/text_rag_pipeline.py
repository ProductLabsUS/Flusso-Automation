...
def text_rag_pipeline(state: TicketState) -> Dict[str, Any]:
    ...
    try:
        ...
        # Add subject for better context
        query = f"{subject}\n\n{text}"
        
        # Query file search
        query_start = time.time()
        hits = client.search_files(query=query, top_k=top_k)
        query_duration = time.time() - query_start
        
        duration = time.time() - start_time
        top_scores = [f"{h.get('score', 0):.3f}" for h in hits[:3]] if hits else []
        
        logger.info(f"{STEP_NAME} | ✅ Complete: {len(hits)} documents in {duration:.2f}s (query: {query_duration:.2f}s)")
        logger.info(f"{STEP_NAME} | 📤 Top scores: {top_scores}")
        
        # Log detailed RAG results
        gemini_answer = hits[0].get("gemini_answer", "") if hits else ""
